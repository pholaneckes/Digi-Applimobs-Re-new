...
public class Ways {
    ...
    public static void setAppmonHealth(AppliEntity appmon, float val){
        appmon.getAttribute(Attributes.MAX_HEALTH).setBaseValue(Math.max(getAppmonHealth(appmon),val));
        appmon.setHealth(val);
    }

    public static String getAppmonAttribute(String name, AppliEntity appli){
        String attri = AppliSetup.AppmonTypes.valueOf(name).getAttributes().getSerializedName();
        if(appli instanceof WaruAppmobs waruAppmob && attri.equals("changable")){
            return AppliAttributes.Attributes.values()[waruAppmob.getWaruType()].getSerializedName();
        }
        return attri;
    }

    public static AppliUnlockedChipItem fromChipGetUnlockedChip(AppliChipItem appliChipItem){
        return (AppliUnlockedChipItem) ForgeRegistries.ITEMS.getValue(new ResourceLocation(AppliUtils.MOD_ID,
                appliChipItem.builtInRegistryHolder().key().location().getPath() + "_unlocked"));
    }
}
